...
import {
	buildDatasets, RandomForestClassifier, RandomForestRegressor,
	CATEGORY_TO_ID, ID_TO_COARSE, ALL_CATEGORIES,
} from './random_forest';
...
function defaultFutureFeatures(trType: TxType, rows: Transaction[]) {
  ...
  // calculate median amount per type from history
	...
	return {
		dow, dom, month, isWeekend, isMonthStart, isMonthEnd, amount: typicalAmount
	};
}
...
function forecastNextForType(
	...
	const { dow, month, amount, isWeekend, isMonthStart, isMonthEnd } =
		defaultFutureFeatures(trType, rows);
